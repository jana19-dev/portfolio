...
<style lang="scss">
  .card {
    ...
    gap: 1rem;
    background: linear-gradient(316deg, #381570d0 0%, #5f0f40c9);
    color: white;
    max-width: 450px;
    height: 100%;
    border-radius: 1rem;
  ...
  a {
    ...
    height: 2rem;
    &:hover {
      background-color: rgb(107, 24, 107);
    }
    svg {
      width: 1rem;
      margin-right: 0.5rem;
...
<div class="card">
  <div class="header">
    <div>
      <h3>{project.title}</h3>
      <h4>{project.date} | {project.subTitle}</h4>
      {#if !project.wip}
        <div class="links">
          {#if project.github}
    ...
    <img src="projects/{project.poster}" alt={project.title} />
  </div>
